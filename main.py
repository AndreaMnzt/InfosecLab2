...
def main():
    ...
    string_with_errors=BSC(stringToBits(bits_string).T[0], error_legitimate_channel)
    length=len(string_with_errors.array)
    ...
    for i in range(number_of_transmission):
        to_be_decrypted=""
        for y in range(7):
            to_be_decrypted+=str((string_with_errors.array)[counter])
            counter+=1
    ...
    u = '010'
    dim = 200
    eps_values = np.linspace(0, 0.5, dim)
    wrong_prob_eps = []
    ...
    for eps in eps_values:
        ...
        for i in range(0, dim):
            ...
            # random encoding
            codeword = randomBinningEncoder(stringToBits(u))
            codeword = codeword.T[0]

            # binary symmetric channel
            string_with_errors = BSC(stringToBits(codeword), eps)

            # decoding
            decoded_u = binningDecoder(string_with_errors.array)[0]
            decoded_str = bitsToString(decoded_u)
            if u != decoded_str:
                cnt_wrong += 1
        wrong_prob_eps.append(cnt_wrong / dim)

    # the histogram of the data z
    plt.plot(eps_values, wrong_prob_eps)
    plt.xlabel("epsilon")
    plt.ylabel("Prob")
